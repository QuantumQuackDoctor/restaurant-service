...
public class OpenApiDocumentationConfig {
  ...
  public Docket customImplementation(ServletContext servletContext,
          @Value("${openapi.orchestrator.base-path:}") String basePath) {
    return new Docket(DocumentationType.SWAGGER_2)
                .select()
                    .apis(RequestHandlerSelectors.basePackage("com.smoothstack.user.api"))
                    .build()
  ...
  class BasePathAwareRelativePathProvider extends RelativePathProvider {
    ...
    protected String applicationPath() {
      return  Paths.removeAdjacentForwardSlashes(UriComponentsBuilder.fromPath(
          super.applicationPath()).path(basePath).build().toString());
    }
